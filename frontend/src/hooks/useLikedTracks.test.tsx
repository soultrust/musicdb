...
describe("useLikedTracks", () => {
  ...
  beforeEach(() => {
    localStorage.clear();
    vi.clearAllMocks();
    vi.mocked(spotifyTracksContains as Mock).mockResolvedValue({
      ok: true,
      json: async () => [false, false],
    });
    vi.mocked(getEspeciallyLikedTracksApi as Mock).mockResolvedValue({
      ok: true,
      data: { tracks: [] },
    });
    vi.mocked(setEspeciallyLikedTrackApi as Mock).mockResolvedValue({ ok: true, data: { ok: true } });
  });
  ...
  it("cycles like state 0 -> 1 -> 2 -> 0 and persists especially-liked", async () => {
    ...
    expect(vi.mocked(setEspeciallyLikedTrackApi as Mock).mock.calls[1][0]).toEqual(
  ...
  it("syncEspeciallyLikedForItem marks server especially-liked tracks as state=2", async () => {
    vi.mocked(getEspeciallyLikedTracksApi as Mock).mockResolvedValueOnce({
      ok: true,
