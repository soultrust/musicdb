...
import type { AuthMode } from "../components/AuthGate";
...
export function useAuth({ API_BASE, AUTH_REFRESH_KEY, onLogoutExtra }: UseAuthParams) {
  ...
  async function handleAuthSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await res.json()) as { error?: string; access?: string; refresh?: string; user?: unknown };
      if (!res.ok) {
