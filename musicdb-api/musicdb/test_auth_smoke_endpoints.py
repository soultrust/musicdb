...
class AuthSmokeEndpointsTests(TestCase):
    ...
    def test_spotify_artist_image_endpoints_require_auth(self):
        ...
        res = self.noauth_client.get("/api/search/spotify-album-search/?q=test")
        self.assertNoAuth(res)

        res = self.noauth_client.get(
            "/api/search/manual-album-image/?musicbrainz_release_group_id=rgid"
        )
        self.assertNoAuth(res)

        res = self.noauth_client.get("/api/search/album-overview/?mbid=rgid")
        self.assertNoAuth(res)
